...
class ScoreEntryWindow(qt.QDialog):
    def __init__(self, main_window, comp_id, db):
        ...
        if len(self.dancers) > 0:
            if self.db.tables.scores.exists_for_event(self.event.id):
                for x in range(len(self.dancers)):
                    score = self.db.tables.scores.get_by_event_dancer(
                        self.event.id, self.dancers[x].id)
        ...
        self.results_box = ResultsGroupBox('Results:', self.event.id, self.db, self.main_window)
    ...
    def save_button(self, sender=None):
        ...
        # print(judge_id)
        if (self.competition.competitionType > 0): # TODO this should be a real check for championship
        ...
        while row < self.table_scores.rowCount():
            ...
            if self.is_float(item_dancer_score.text()):
                ...
                score = sc.Score(0, dancer_id, self.event.id, judge_id,
